...
`robots_txt_snapshot.txt` holds the same file as plain text with its retrieval
time recorded at the top — an image cannot be grepped or diffed, and the live
file changes over time, so this is the record of the rules the scraper was
...
(No browser is involved in scraping at all. `screenshot.jpg` was captured by
viewing robots.txt in a browser, which is evidence-gathering, not scraping.)
...
module_2/
  ...
  robots_txt_snapshot.txt      robots.txt as plain text, with retrieval timestamp
  requirements.txt
  README.md
